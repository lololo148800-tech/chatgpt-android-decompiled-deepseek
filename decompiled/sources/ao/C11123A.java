package ao;

import kotlin.jvm.internal.AbstractC16544l;
import p631Zn.InterfaceC10428a;
import p631Zn.InterfaceC10429b;

/* JADX INFO: renamed from: ao.A */
/* JADX INFO: loaded from: classes2.dex */
public final class C11123A extends AbstractC11161e0 {

    /* JADX INFO: renamed from: c */
    public static final C11123A f33742c = new C11123A(C11125B.f33744a);

    @Override // ao.AbstractC11152a
    /* JADX INFO: renamed from: d */
    public final int mo12361d(Object obj) {
        float[] fArr = (float[]) obj;
        AbstractC16544l.m18094g(fArr, "<this>");
        return fArr.length;
    }

    @Override // ao.AbstractC11186r, ao.AbstractC11152a
    /* JADX INFO: renamed from: f */
    public final void mo12362f(InterfaceC10428a interfaceC10428a, int i10, Object obj, boolean z6) {
        C11202z builder = (C11202z) obj;
        AbstractC16544l.m18094g(builder, "builder");
        float fMo5467E = interfaceC10428a.mo5467E(this.f33800b, i10);
        builder.mo12371b(builder.mo12372d() + 1);
        float[] fArr = builder.f33869a;
        int i11 = builder.f33870b;
        builder.f33870b = i11 + 1;
        fArr[i11] = fMo5467E;
    }

    @Override // ao.AbstractC11152a
    /* JADX INFO: renamed from: g */
    public final Object mo12363g(Object obj) {
        float[] fArr = (float[]) obj;
        AbstractC16544l.m18094g(fArr, "<this>");
        C11202z c11202z = new C11202z();
        c11202z.f33869a = fArr;
        c11202z.f33870b = fArr.length;
        c11202z.mo12371b(10);
        return c11202z;
    }

    @Override // ao.AbstractC11161e0
    /* JADX INFO: renamed from: j */
    public final Object mo12364j() {
        return new float[0];
    }

    @Override // ao.AbstractC11161e0
    /* JADX INFO: renamed from: k */
    public final void mo12365k(InterfaceC10429b encoder, Object obj, int i10) {
        float[] content = (float[]) obj;
        AbstractC16544l.m18094g(encoder, "encoder");
        AbstractC16544l.m18094g(content, "content");
        for (int i11 = 0; i11 < i10; i11++) {
            encoder.mo5578l(this.f33800b, i11, content[i11]);
        }
    }
}
